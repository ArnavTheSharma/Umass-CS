...
public class LargestPageComparator implements Comparator<WebPageRecord> {
    ...
    public int compare(WebPageRecord x, WebPageRecord y) {
        ...
        // if URL lengths are equal it'll continue to below
        int xLen = (x.firstLine == null) ? 0 : x.firstLine.length();
        int yLen = (y.firstLine == null) ? 0 : y.firstLine.length();    
        if (xLen != yLen) {
            return Integer.compare(yLen, xLen);
        } 
   
        // if first line lengths are equal it'll continue to below

        CasedURLComparator c = new CasedURLComparator(false);
