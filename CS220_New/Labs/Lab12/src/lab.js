...
export function getObjsWithName(urls) {
  return Promise.allSettled(
    // settles to a single promise of an Array of responses
    urls.map(url => {
      return fetch(url)
    ...
    // keep the ones that are fulfilled (res.status) and then map responses to their res["value"] then filter the ones for "name" in obj and .flat() to flatten into 1 array
    return resArr
